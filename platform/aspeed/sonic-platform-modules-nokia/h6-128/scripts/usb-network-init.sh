...
# Bring interface down (required for IPv6)
ip link set "${INTERFACE_NAME}" down

# Enable IPv6 on the interface
sysctl -w net.ipv6.conf."${INTERFACE_NAME}".autoconf=1 2>/dev/null || true
...
#TODO: This temoporary workaround will be removed once
...
try:
    ...
    if bmc_ip and netmask:
        ...
        print(f'{interface.with_prefixlen}')
    else:
        sys.exit(1)
