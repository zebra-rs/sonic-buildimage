...
class PIDFanController():
    """
    Common FAN PID controller for CPU and Switch Temp
    """
    MAX_SPEED = 255
    MIN_SPEED = 89
    ...
    def load_from_json(self, json_obj):
        ...
        if self.THERMAL_LOG_LEVEL in json_obj:
            ...
            if not isinstance(thermal_log_level, int) or thermal_log_level not in range(0,8):
                raise ValueError('ThermalAlgorithmAction invalid thermal log level, a integer in range 0-7 is required')
        ...
        sonic_logger.log_info("[ThermalAlgorithmAction] cpu_pid: {}, f2b_linear: {}, b2f_linear: {}".format(self.cpu_pid_params, self.f2b_linear_params, self.b2f_linear_params))
    ...
    def execute(self, thermal_info_dict):
        ...
        if ThermalInfo.INFO_NAME in thermal_info_dict and \
           isinstance(thermal_info_dict[ThermalInfo.INFO_NAME], ThermalInfo):
            ...
            cpu_thermal_data = thermals_data["CPU"]
            cpu_fan_pwm = self.cpu_fan_controller.calc_fan_speed(cpu_thermal_data)
            thermal_data = thermals_data["MAC Heatsink"]
            linear_fan_pwm1 = self.linear_fan_controller.calc_fan_speed(thermal_data)
            thermal_data = thermals_data["PSU-A Intake"]
            linear_fan_pwm2 = self.linear_fan_controller.calc_fan_speed(thermal_data)
            thermal_data = thermals_data["PSU-B Intake"]
            linear_fan_pwm3 = self.linear_fan_controller.calc_fan_speed(thermal_data)
            target_fan_pwm = max(cpu_fan_pwm, linear_fan_pwm1, linear_fan_pwm2, linear_fan_pwm3)
            sonic_logger.log_info("[ThermalAlgorithmAction] cpu_pid_pwm: {}, linear_fan_pwm: {}, linear_fan_pwm2: {}, linear_fan_pwm3: {}, target_pwm: {}".format(cpu_fan_pwm, linear_fan_pwm1, linear_fan_pwm2, linear_fan_pwm3, target_fan_pwm))
