...
class Chassis(PddfChassis):
    ...
    def get_reboot_cause(self):
        ...
        if description == "Unknown":
            try:
                q = subprocess.check_output('journalctl -b | grep "Previous system reset reason"', shell=True)
                reason = q.decode()
                if "remote ASF power off" in reason:
                    reboot_cause = self.REBOOT_CAUSE_POWER_LOSS
                    description = 'Power Cycle Reset'
                elif "system reset pin" in reason:
                    reboot_cause = self.REBOOT_CAUSE_WATCHDOG
                    description = 'Hardware Watchdog Reset'
                elif "software wrote 0x6" in reason:
                    reboot_cause = self.REBOOT_CAUSE_NON_HARDWARE
                    description = 'Soft-Set Cold Reset'
                elif "software wrote 0x4" in reason:
                    reboot_cause = self.REBOOT_CAUSE_NON_HARDWARE
                    description = 'Soft-Set Warm Reset'
                elif "software wrote 0xE" in reason:
                    reboot_cause = self.REBOOT_CAUSE_NON_HARDWARE
                    description = 'Soft-Set Warm Reset'
                else:
                    reboot_cause = self.REBOOT_CAUSE_POWER_LOSS
